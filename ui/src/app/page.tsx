...
import { api, TimeEntry, ProjectStats } from '@/lib/api';
                ...
                <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                  {recentEntries.map((entry) => (
                    <tr key={entry.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                      <td className="px-6 py-4 whitespace-nowrap font-medium text-gray-900 dark:text-white capitalize">{entry.project}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600 dark:text-gray-400">{formatDateTime(entry.start_time)}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600 dark:text-gray-400">{formatDateTime(entry.end_time)}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold text-gray-900 dark:text-white">{calculateDuration(entry.start_time, entry.end_time)}h</td>
                      <td className="px-6 py-4 text-sm text-gray-600 dark:text-gray-400 max-w-xs truncate">{entry.notes || '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
